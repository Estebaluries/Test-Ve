...
function setupWhatsAppButtons() {
  ...
  // Evento para el botón de WhatsApp en el formulario (si el botón existe)
  if (whatsappBtn) {
    whatsappBtn.addEventListener('click', function () {
      const nameEl = document.getElementById('name');
      const serviceSelect = document.getElementById('service');
      const name = (nameEl && nameEl.value) || 'Cliente';
  ...
  if (serviceForm) {
    serviceForm.addEventListener('submit', function (e) {
      ...
      alert(
        'Su solicitud ha sido registrada. Un agente del Ministerio de Ve se pondrá en contacto en breve.'
      );
      this.reset();
    });
  }
